...
USceneCapturer::USceneCapturer(FVTableHelper& Helper)
    ...
    , hAngIncrement(FKilographStereoPanoramaManager::HorizontalAngularIncrement->GetFloat())
    , vAngIncrement(FKilographStereoPanoramaManager::VerticalAngularIncrement->GetFloat())
    , eyeSeparation(FKilographStereoPanoramaManager::EyeSeparation->GetFloat())
    ...
    , SphericalAtlasWidth(FKilographStereoPanoramaManager::StepCaptureWidth->GetInt())
    , SphericalAtlasHeight(SphericalAtlasWidth / 2)
    , bForceAlpha(FKilographStereoPanoramaManager::ForceAlpha->GetInt() != 0)
    , bEnableBilerp(FKilographStereoPanoramaManager::EnableBilerp->GetInt() != 0)
    , SSMethod(FMath::Clamp<int32>(FKilographStereoPanoramaManager::SuperSamplingMethod->GetInt(), 0, ARRAY_COUNT(g_ssPatterns)))
    , bOverrideInitialYaw(FKilographStereoPanoramaManager::ShouldOverrideInitialYaw->GetInt() != 0)
    , ForcedInitialYaw(FRotator::ClampAxis(FKilographStereoPanoramaManager::ForcedInitialYaw->GetFloat()))
    , OutputDir(FKilographStereoPanoramaManager::OutputDir->GetString().IsEmpty() ? FPaths::GameSavedDir() / TEXT("StereoPanorama") : FKilographStereoPanoramaManager::OutputDir->GetString())
    , dbgDisableOffsetRotation(FKilographStereoPanoramaManager::FadeStereoToZeroAtSides->GetInt() != 0)
{}
    ...
    , dbgDisableOffsetRotation( FKilographStereoPanoramaManager::FadeStereoToZeroAtSides->GetInt() != 0 )
{
    ...
    if (dbgMatchCaptureSliceFovToAtlasSliceFov)
    {
        ...
        captureHFov = FKilographStereoPanoramaManager::CaptureHorizontalFOV->GetFloat();
    ...
    else
    {
        ...
        captureVFov = captureHFov = FKilographStereoPanoramaManager::CaptureHorizontalFOV->GetFloat();
        ...
        CaptureHeight = CaptureWidth = FKilographStereoPanoramaManager::CaptureSlicePixelWidth->GetInt();
	...
	for( int CaptureIndex = 0; CaptureIndex < FKilographStereoPanoramaManager::ConcurrentCaptures->GetInt(); CaptureIndex++ )
	{
